...
#### 重要な注意点：Monaco トークナイザーの状態管理

**CRITICAL**: Monaco Editor のトークナイザーは状態マシンとして動作します。状態遷移を誤ると、**行をまたいで状態が引き継がれ、以降のすべての行が間違った色でハイライトされる**という致命的な問題が発生します。

##### よくある落とし穴

1. **`@pop` を忘れる**
   - 状態に入ったら、必ず行末で `@pop` して root 状態に戻る必要があります
   - `@pop` がないと、次の行も同じ状態で処理されてしまいます

2. **複数の状態スタック**
   - 状態Aから状態Bに遷移した場合、状態Bから root に戻るには `@popall` が必要です
   - 例: `root` → `@textLine` → `@textLineContent` の場合、`textLineContent` から root に戻るには `@popall` を使います

3. **行末マッチングの落とし穴**
   - `/[^\/]+$/` のようなパターンは意図通りに動作しないことがあります
   - Monaco のトークナイザーは行ごとに処理するため、`$` が期待通りにマッチしない場合があります
   - `@rematch` を使って再マッチングさせることで解決できます

##### 推奨パターン：@rematch を使った確実な処理

```javascript
// 悪い例：状態から抜けられない可能性がある
textLine: [
  [/\/\/.*$/, "comment", "@pop"],
  [/[^\/]+/, "string"],  // 行末に達しても pop しない
  [/$/, "", "@pop"],      // このパターンが機能しないことがある
],
...
1. **色が間違っている箇所を見つける**
   - `snapshot/*.view` で `class="mtk*"` と `style="color: rgb(*)"` を確認
   - 例: キーワードが `mtk9` (青) ではなく `mtk5` (オレンジ) になっている場合、状態が残っている可能性

2. **どの行から色が変わったかを特定**
   - おかしくなった行の直前の行を確認
   - その行で状態遷移が正しく終了しているか（`@pop` or `@popall` があるか）確認

3. **パターンマッチングの順序を確認**
   - Monaco は上から順にパターンをマッチング
   - より具体的なパターンを先に、汎用的なパターンを後に配置

4. **テストケースを追加**
   - 問題が発生するパターンを `spec/test.gbs` に追加
   - `npm run capture-syntax` で確認

##### 実例：メッセージ行の途中コメント対応

テキスト行（`>` または `-` で始まる）では、メッセージとコメントを分離する必要がありました：

```
> メッセージテキスト // コメント
```

最初の実装では状態から抜けられず、次の行以降すべてが string 色になってしまいました。
`@rematch` と `@popall` を使うことで解決しました。
